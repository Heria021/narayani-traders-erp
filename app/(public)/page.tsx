...
const PROJECT_ART = [
  // 1 — grid of rectangles (pavilion / glass)
  ...
  // 3 — diagonal cross-hatch (penthouse / high ceiling)
  `M 20 20 L 220 220
   M 60 20 L 220 180
   M 100 20 L 220 140
   M 140 20 L 220 100
  ...
  // 7 — vertical risers (high-rise / tower)
  `M 40 220 L 40 20
   M 80 220 L 80 40
   M 120 220 L 120 20
   M 160 220 L 160 40
...
export default function PublicHomePage() {
  ...
  // ── RAFloop for progress bar — zero React re-renders ─────────────────────
  ...
  if (prevIndexRef.current !== activeIndex) {
    ...
    stopLoop()
    // defer so React has committed the new slide
  ...
  return (
    <div className="bg-black text-white w-full min-h-screen">
      ...
      <section className="relative h-screen w-full overflow-hidden bg-black">
        ...
        {/* BG images — all mounted, only opacity toggled — zero layout cost */}
      ...
      {/* ══════════════════════════════════════════
          PROJECT ARCHIVE
      ══════════════════════════════════════════ */}
      <section id="projects" className="w-full bg-[#060606] py-24 md:py-32 px-6 md:px-12 border-t border-white/8">
        <div className="max-w-6xl mx-auto flex flex-col">
          ...
          {/* ── Alternating card list with centre spine line ── */}
          <div className="relative">

            {/* Centre spine — vertical line connecting all cards */}
            <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-px -translate-x-1/2" style={{ background: "linear-gradient(to bottom, transparent, rgba(255,255,255,0.12) 5%, rgba(255,255,255,0.12) 95%, transparent)" }} />
...
// ─── Project Row ──────────────────────────────────────────────────────────────
// Each row is the full-width unit: [photo card] [spine dot] [art card]
// or reversed when isLeft=false. The row slides in from its respective side
// bidirectionally (both on scroll-down and scroll-up).
...
function ProjectRow({
  ...
  const isInView = useInView(rowRef, { margin: "-12% 0px -12% 0px", amount: 0 })

  const photoX  = isLeft ? "-8%" : "8%"
  const artX    = isLeft ? "8%"  : "-8%"

  return (
    <motion.div
      ref={rowRef}
      className="relative grid grid-cols-1 md:grid-cols-[1fr_48px_1fr] items-center gap-0 py-8 md:py-10"
    >
      {/* ── Spine dot + connector lines ── */}
      <div className="hidden md:flex absolute left-1/2 top-0 bottom-0 -translate-x-1/2 flex-col items-center pointer-events-none z-10">
        {/* Line from top to dot */}
        <motion.div
          className="w-px flex-1"
          style={{ background: "rgba(255,255,255,0.12)" }}
          animate={{ scaleY: isInView ? 1 : 0, originY: 0 }}
          transition={{ duration: 0.5, ease: EASE_EXPO }}
        />
        {/* Dot */}
        <motion.div
          className="w-2.5 h-2.5 rounded-full border border-white/40 bg-[#060606] shrink-0 my-1"
          animate={{ scale: isInView ? 1 : 0, opacity: isInView ? 1 : 0 }}
          transition={{ duration: 0.35, ease: EASE_EXPO, delay: 0.15 }}
        />
      ...
      >
        {isLeft ? (
          ...
          <ArtCard project={project} artPaths={artPaths} />
        )}
      </motion.div>

      {/* Spine spacer (desktop) */}
      ...
      <motion.div
        ...
        transition={{ duration: 0.72, ease: EASE_EXPO, delay: 0.08 }}
      >
        {isLeft ? (
          <ArtCard project={project} artPaths={artPaths} />
        ) : (
...
// ─── Photo Card ───────────────────────────────────────────────────────────────
// Clean hover: image scale + overlay darken + content rise. No 3D.
...
function PhotoCard({ project }: { project: Project }) {
  ...
  // Subtle parallax shift on the bg image — purely motion values, no setState
  ...
  return (
    <motion.article
      ...
      className="group relative overflow-hidden rounded-2xl bg-[#0d0d0d] border border-white/8"
      style={{ height: 420 }}
      whileHover="hover"
      initial="rest"
    >
      ...
      {/* Content — rises slightly on hover */}
      <motion.div
        className="absolute inset-0 z-20 flex flex-col justify-end p-7"
        variants={{ rest: { y: 0 }, hover: { y: -6 } }}
        transition={{ duration: 0.45, ease: EASE_EXPO }}
      >
        <span className="text-[9px] font-bold tracking-[0.24em] text-white/45 uppercase mb-2">
          {project.category} &nbsp;·&nbsp; {project.year}
        </span>
        <h3 className="text-2xl font-light tracking-wide text-white uppercase leading-tight mb-1">
          {project.title}
        </h3>
        ...
        <motion.p
          ...
          variants={{ rest: { opacity: 0, y: 8, height: 0 }, hover: { opacity: 1, y: 0, height: "auto" } }}
      ...
      {/* Corner accent line — draws in on hover */}
      <motion.div
...
// ─── Art Card ─────────────────────────────────────────────────────────────────
// SVG line art that draws itself (stroke-dashoffset) on hover.
// Shows project meta on the left, art panel on the right inside the card.

function ArtCard({ project, artPaths }: { project: Project; artPaths: string }) {
  const [hovered, setHovered] = useState(false)

  // Split compound path into individual paths for staggered draw
  const paths = artPaths.trim().split("\n").map(p => p.trim()).filter(Boolean)

  return (
    <motion.article
      className="group relative overflow-hidden rounded-2xl border border-white/8 bg-[#0d0d0d]"
      style={{ height: 420 }}
    ...
    >
      ...
      <div className="relative h-full flex flex-col md:flex-row">
        ...
        {/* ── Left meta panel ── */}
        <div className="flex flex-col justify-between p-7 md:w-[45%] shrink-0 border-r border-white/6">

          <div>
            <span className="text-[9px] font-bold tracking-[0.24em] text-white/35 uppercase block mb-5">
              {project.category} &nbsp;·&nbsp; {project.year}
            </span>
            <h3 className="text-xl font-light tracking-wide text-white uppercase leading-tight">
              {project.title}
            </h3>
          </div>
          ...
          <div className="flex flex-col gap-5 mt-6">
            ...
            <div className="flex flex-col gap-2.5">
              ...
              <div className="text-[10px] text-white/25 pl-5">{project.area}</div>
            </div>

            <Link href={`/projects/${project.slug}`}
              className="inline-flex items-center gap-1.5 text-[10px] font-semibold tracking-widest uppercase text-white/55 hover:text-white transition-colors duration-200 mt-1"
              onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 relative overflow-hidden flex items-center justify-center p-4">
          ...
          {/* Grid lines background — always visible, very subtle */}
          ...
          {/* Project index number — large, ghosted */}
          <span className="absolute bottom-4 right-5 text-[72px] font-bold text-white/[0.04] leading-none select-none pointer-events-none">
            {String(project.id).padStart(2, "0")}
          </span>

          {/* ── SVG line art — stroke-dashoffset draw animation ── */}
          <svg
            viewBox="0 0 240 240"
...
function ArtPath({ d, hovered, delay }: { d: string; hovered: boolean; delay: number }) {
  ...
  // We compute length once after mount and store it
  const lengthRef = useRef<number>(200)

  // Measure path length after first render
  const measuredRef = useRef(false)
  if (!measuredRef.current && typeof window !== "undefined") {
    // Will be measured on first paint via useEffect equivalent below
  }
  ...
  return (
    <motion.path
      ...
      // Use pathLength for normalised dash animation — no need to measure
      pathLength={1}
      strokeDasharray="1"
      animate={{
